...
from be.api.v1.models.response_models import (
...
class CodeTutorialService:
    def __init__(
        self, content_client: ContentGenClient, supabase_client: SupabaseWrapper
    ):
        self.content_client = content_client
        self.supabase_client = supabase_client

    async def create_new_tutorial(
        self, tutorial_context: TutorialContext, concept: str
    ) -> NewCodeTutorialResponse:
        # ask content client for a single question
        question_response = await self.content_client.generate_question(
            tutorial_context, concept
        )
        ...
        tutorial = CodeTutorial(
            uuid=tutorial_uuid, questions=[], context=tutorial_context
        )

        # unique-ify the question and add it to the tutorial
        unique_code_question = UniqueCodeQuestion(
            uuid=uuid.uuid4(), question=question_response.code_question
        )
    ...
    async def get_hint(
        ...
        hint = await self.content_client.generate_hint(
            question.question, tutorial.context, user_code
        )
        return HintResponse(hint_text=hint.text)

    async def give_up(
        self, tutorial_uuid: uuid, question_uuid: uuid, user_code: CodeBlock
    ) -> GiveUpResponse:
        # fetch the tutorial from supabase
        tutorial = await self.supabase_client.get_tutorial(tutorial_uuid)
        if tutorial is None:
        ...
        # ask content client for a hint
        give_up_message = await self.content_client.generate_give_up(
            question.question,
            tutorial.context,
            user_code,
            question.question.solution_code,
        )
        return GiveUpResponse(
            explanation=give_up_message.text,
    ...
    async def more_questions(self, tutorial_uuid) -> MoreQuestionsResponse:
        ...
        # ask content client for a single question
        question_response = await self.content_client.generate_question(
            tutorial.context, tutorial.questions[0].question.concept
        )
    ...
    async def get_affirmation(
        self, tutorial_uuid: uuid, attempts_taken: int
    ) -> PositiveAffirmationResponse:
        # fetch the tutorial from supabase
        tutorial = await self.supabase_client.get_tutorial(tutorial_uuid)
        if tutorial is None:
        ...
        # ask content client for a single question
        affirmation_response = await self.content_client.generate_affirmation(
            tutorial.context, attempts_taken
        )
    ...
    async def report_question(
        ...
        reported_question = ReportedQuestion(
            uuid=uuid.uuid4(),
            reported_question=bad_question,
            category=category,
            details=details,
            was_regenerated=should_regenerate,
        )
        ...
        if should_regenerate:
            # ask content client for a single question
            new_question_response = await self.content_client.generate_question(
                tutorial.context, tutorial.questions[0].question.concept
            )

            # unique-ify the question and add it to the tutorial and to the response
            new_question = UniqueCodeQuestion(
                uuid=uuid.uuid4(), question=new_question_response.code_question
            )
