...
class SupabaseWrapper:
    ...
    async def oauth_sign_in(self, provider: str, scopes: str = "user"):
        data = await asyncio.to_thread(
            lambda: self.supabase_client.auth.sign_in_with_oauth(
                {
                    "provider": provider,
                    "options": {
    ...
    async def insert_tutorial(self, tutorial: CodeTutorial) -> CodeTutorial | None:
        # no async support without rolling our own: https://github.com/supabase-community/supabase-py/issues/250
        response = await asyncio.to_thread(
            lambda: self.supabase_client.table("tutorials")
            .insert(json.loads(tutorial.json()))
            .execute()
        )
    ...
    async def update_tutorial(self, tutorial: CodeTutorial) -> CodeTutorial | None:
        # no async support without rolling our own: https://github.com/supabase-community/supabase-py/issues/250
        response = await asyncio.to_thread(
            lambda: self.supabase_client.table("tutorials")
            .update(json.loads(tutorial.json()))
            .eq("uuid", tutorial.uuid)
            .execute()
        )
        if response.data is None or len(response.data) == 0:
            return None
    ...
    async def insert_question(
        self, question: UniqueCodeQuestion
    ) -> UniqueCodeQuestion | None:
        # no async support without rolling our own: https://github.com/supabase-community/supabase-py/issues/250
        response = await asyncio.to_thread(
            lambda: self.supabase_client.table("questions")
            .insert((json.loads(question.json())))
            .execute()
        )
        if response.data is None or len(response.data) == 0:
            return None
    ...
    async def update_question(
        ...
        response = await asyncio.to_thread(
            lambda: self.supabase_client.table("questions")
            .update(json.loads(question.json()))
            .eq("uuid", question.uuid)
            .execute()
        )
        if response.data is None or len(response.data) == 0:
            return None
    ...
    async def insert_reported_question(
        self, reported_question: ReportedQuestion
    ) -> ReportedQuestion | None:
        # no async support without rolling our own: https://github.com/supabase-community/supabase-py/issues/250
        response = await asyncio.to_thread(
            lambda: self.supabase_client.table("reported_questions")
            .insert((json.loads(reported_question.json())))
            .execute()
        )
        if response.data is None or len(response.data) == 0:
            return None
        return ReportedQuestion.parse_obj(response.data[0])
